...
以下の保険提案トークの内容から、営業現場で即活用できる重要ポイントを抽出し、簡潔に要約してください。
...
- 200-300文字程度の非常に簡潔な要約にしてください
- 各見出しの内容は文章形式で、読みやすく構成してください
- 会話の具体例ではなく、トーク全体のエッセンスを抽出してください

【表現方法】
- Markdown形式は使用できません
- 強調したい部分は『』で囲んでください（例：『効果的な質問』）
- 箇条書きは・を使用してください(例：・主要ポイント)
- 見出しやタイトルは［ ］で囲んでください（例：［話題展開］）
...
注意事項：
- 具体的な会話内容ではなく、営業トークの重要なポイントを抽出してください
- 複数の提案パターンがある場合は、それぞれの特徴や違いがわかるようにまとめてください
- 実際の営業場面で活用できる実践的な内容に絞ってください
- 専門用語は避け、わかりやすい表現を使用してください
"""
        return prompt