...
1. メリット・デメリット分析
- 現在の保険商品と乗り換え先商品の主要な違い
- 保障内容の比較における最重要な優位点・懸念点
- 保険料に関する比較の要点

2. 乗り換え提案の方法論
- 最も効果的な提案の流れを簡潔に
- 最重要な説明ポイント
- 1つの想定される顧客状況と対応する提案セリフの例
...
【出力形式】
必ず以下の正確な形式で出力してください。各タグは省略せず必ず含めてください：
...
注意事項：
- 各セクションの合計で300文字程度になるよう簡潔に記述すること
- 最重要ポイントのみを抽出し、詳細な説明は省略すること
...
提供された保険商品の内容を評価し、十分な情報が含まれていない場合を検知してください。
...
各フェーズでは、保険営業員とお客様の最小限の必要なやり取りを作成してください：
...
3. ニーズ喚起フェーズ
- お客様のニーズ喚起をする質問を組み込む
- お客様の回答から保険ニーズを引き出す簡潔なやり取り

4. 切込み提案フェーズ
- 保険提案を実際に行う切込みセリフを組み込む
- 提案保険カテゴリへの興味を引き出す最小限の会話
...
各フェーズで1-2往復程度の必要最小限の会話を生成してください。
...
【注意事項】
- 出力全体で300文字程度に収まるよう簡潔にしてください
- 時事ネタを一つに絞り、提案パターンを一つだけ出力してください
- 各フェーズは最小限の会話で効果的に表現してください
- お客様の性格や状況に合わせた簡潔な反応を意識してください
- 押しつけがましくならない、自然かつ簡潔な会話の流れを作ってください
- 冗長な表現や不要な説明は避けてください
- タグは必ず省略せずに含めてください
"""
        return prompt
...
以下の乗り換え提案の内容を、最も重要な1-2点に絞って簡潔に要約してください。
...
1. 乗り換えの決定的根拠
   - 乗り換えによる最も重要なメリット
   - または乗り換えを見送るべき最も重要な理由

2. 営業のための最重要ポイント
   - 最も効果的な提案アプローチまたは反論対応

【要約条件】
- 100-150文字以内の非常に簡潔な要約にしてください
- 提案の本質的な価値のみを抽出してください
- 細部の説明は一切省略してください
...
【出力形式】
必ず以下の正確な形式で出力してください。タグは省略せず必ず含めてください：
...
注意事項：
- 元のテキストは既に300文字程度に要約されています
- 重要なポイントを抽出してください
- 営業現場で即座に使える、核心を突いた内容にしてください
- タグは必ず省略せずに含めてください
"""
        return prompt

   def get_talk_summary_proposal_prompt(self, talk_text: str) -> str:
        """
        保険提案トークの内容を要約するためのプロンプト
        
        Args:
            talk_text (str): 保険提案トークの内容
            
        Returns:
            str: 要約生成用のプロンプト
        """
        prompt = f"""あなたは保険提案トークを要約する専門家です。
以下の保険提案トークの内容から、最も重要な1-2点のみを抽出し、簡潔に要約してください。

【提案トーク】
{talk_text}

以下の要素のうち最も重要な1-2点のみを選んで含めてください：

1. トーク全体の核心
   - 時事ネタと保険提案の最も効果的な結びつけ方
   - または最も重要な会話展開のポイント
...
注意事項：
- 元のテキストは既に300文字程度に要約されています
- さらに最重要ポイントのみを抽出してください
- 会話の細部や表現ではなく、核心的な技法や考え方に焦点を当ててください
- 即座に営業現場で実践できる内容を優先してください
- タグは必ず省略せずに含めてください
"""
        return prompt